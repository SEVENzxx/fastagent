...
import type { ProductImportResponse } from '@/api/products'
...
const result = ref<ProductImportResponse | null>(null)
...
async function downloadTemplate() {
  try {
    const blob = await productsApi.downloadProductImportTemplate()
...
async function handleImport() {
  ...
  try {
    result.value = await productsApi.importProducts(selectedFile.value)
    if (result.value.success) {
      ElMessage.success(`成功导入 ${result.value.createdCount} 个商品`)
      emit('imported')
    } else {
...
<template>
  ...
  >
    <div class="import-body">
      ...
      <el-alert
        title="导入前会校验商品名称、SKU、分类路径、价格、库存和规格 JSON。发现任意错误时，整批商品都不会写入。"
        type="info"
        show-icon
        :closable="false"
      />
      ...
      <el-alert
        v-if="result?.success"
        :title="`导入完成：共 ${result.totalRows} 行，成功创建 ${result.createdCount} 个商品`"
        type="success"
        show-icon
        :closable="false"
      />
