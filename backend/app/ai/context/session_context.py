...
class SessionContext(BaseModel):
    ...
    # ── 租户与会话标识 ──
    tenant_id: int = Field(0, description="租户 ID")
    conversation_id: int = Field(0, description="当前会话 ID")
    contact_id: int | None = Field(None, description="当前客户联系人 ID")

    # ── 上一轮路由信息（用于上下文连贯和消歧） ──
    last_skill: str | None = Field(None, description="上一轮调用的 Skill")
    last_intent: str | None = Field(None, description="上一轮识别的用户意图")
    turn_count: int = Field(0, description="当前会话已交互轮数")
    last_user_message: str | None = Field(None, description="用户上一轮发送的原始消息文本")

    # ── 商品域上下文（当前浏览/比较的商品信息） ──
    active_product_ids: list[str] = Field(default_factory=list, description="当前对话活跃商品 ID 列表")
    active_product_names: list[str] = Field(default_factory=list, description="当前对话活跃商品名称列表")
    last_product_id: str | None = Field(None, description="最近一次引用的商品 ID（指代解析用）")
    last_product_name: str | None = Field(None, description="最近一次引用的商品名称")
    last_focus_product_id: str | None = Field(None, description="当前焦点商品 ID（等价于 last_product_id）")
    ...
    # ── 知识域上下文（用于追问续查和精准召回） ──
    last_knowledge_topic: str | None = Field(None, description='知识主题（如"优惠活动", "保修政策"）')
    last_knowledge_scope: str | None = Field(None, description="来源范围: qa / knowledge")
    last_knowledge_refs: list[dict] = Field(default_factory=list, description="最近知识引用，最多 5 条")

    # ── 元数据 ──
    updated_at: str | None = Field(None, description="上下文最近更新时间（ISO 8601）")
