...
class KnowledgeDoc(Base):
    ...
    file_type: Mapped[str] = mapped_column(String(30), nullable=False, comment="文件类型: pdf/docx/md/txt/html")
    ...
    status: Mapped[str] = mapped_column(String(30), nullable=False, default="processing", server_default="processing", comment="状态: processing/ready/failed")
    chunk_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0, server_default="0", comment="分块总数")
    product_id: Mapped[int | None] = mapped_column(BigInteger, ForeignKey("products.id", ondelete="SET NULL"), nullable=True, comment="关联商品ID")
