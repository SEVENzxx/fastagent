...
class Tenant(Base):
    ...
    slug: Mapped[str] = mapped_column(String(100), nullable=False, comment="URL 标识")
    plan_id: Mapped[int | None] = mapped_column(BigInteger, ForeignKey("plans.id"), comment="当前套餐")
    plan_expires_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), comment="套餐到期时间")
    custom_prompt: Mapped[str | None] = mapped_column(Text, comment="AI 人设 Prompt")
    llm_provider: Mapped[str | None] = mapped_column(String(50), comment="openai / oneapi / deepseek")
    llm_api_key_encrypted: Mapped[str | None] = mapped_column(Text, comment="加密存储的 API Key")
    llm_model: Mapped[str | None] = mapped_column(String(100), comment="使用的模型名")
    oneapi_base_url: Mapped[str | None] = mapped_column(String(500), comment="One-API 网关地址")
